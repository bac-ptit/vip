...
[[nodiscard]] inline drogon::Task<Product> Create(const Product& p) {
...
[[nodiscard]] inline drogon::Task<std::optional<Product>> FindById(std::string id) {
  auto db{drogon::app().getFastDbClient()};
  drogon::orm::CoroMapper<ProductsModel> mapper{db};
  try {
...
[[nodiscard]] inline drogon::Task<std::vector<Product>> FindAll() {
  auto db{drogon::app().getFastDbClient()};
  drogon::orm::CoroMapper<ProductsModel> mapper{db};
  auto res{co_await mapper.findAll()};
  co_return res |
            std::views::transform([](auto& item) { return Product{std::move(item)}; }) |
            std::ranges::to<std::vector>();
}

[[nodiscard]] inline drogon::Task<std::vector<Product>> FindActive() {
  auto db{drogon::app().getFastDbClient()};
  drogon::orm::CoroMapper<ProductsModel> mapper{db};
...
[[nodiscard]] inline drogon::Task<size_t> Update(const Product& p) {
  auto db{drogon::app().getFastDbClient()};
  drogon::orm::CoroMapper<ProductsModel> mapper{db};
...
[[nodiscard]] inline drogon::Task<bool> DeleteById(std::string id) {
  ...
  auto count{co_await mapper.deleteBy(drogon::orm::Criteria(
      ProductsModel::Cols::_id, drogon::orm::CompareOperator::EQ,
      std::move(id)))};
  co_return count > 0;
}
